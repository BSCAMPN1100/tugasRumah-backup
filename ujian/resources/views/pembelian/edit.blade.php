...
@section('title', 'Edit Pembelian · CatatRezekimu')
        ...
        <div class="bg-white rounded-3xl shadow-lg p-5">
            ...
            <div class="flex justify-between items-center mb-4">
                <div>
                    ...
                    <h1 class="text-2xl font-bold text-[#0b3b2c] mt-1">✏️ Edit Pembelian #{{ $pembelian->id }}</h1>
                </div>
            ...
            <div class="mb-4">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <span class="text-sm text-gray-500" id="total-item-label">{{ $pembelian->detail->count() }} item</span>
                </div>
            ...
            <div class="bg-[#f1f5f9] rounded-2xl p-4 mb-5">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <div class="text-xl font-bold text-gray-800" id="total-item-count">{{ $pembelian->detail->sum('jumlah') }}</div>
                    </div>
                    <div class="text-right">
                        <div class="text-sm text-gray-600">Total Pembelian</div>
                        <div class="text-2xl font-bold text-[#0b3b2c]" id="total-harga">Rp {{ number_format($pembelian->total_harga, 0, ',', '.') }}</div>
                    </div>
                </div>
            </div>

            <button id="btn-simpan" class="btn-primary text-lg py-4" onclick="updatePembelian()">
                💾 Update Pembelian
            </button>
...
<script>
    ...
    @foreach($pembelian->detail as $detail)
        daftarPembelian.push({
            ...
            harga_modal: {{ $detail->harga_modal_saat_beli }},
    ...
    function tambahBarangKeDaftar() {
        ...
        if (!select.value) { alert('Silakan pilih barang terlebih dahulu!'); return; }
        ...
        if (existing) {
            ...
            existing.subtotal = existing.harga_modal * existing.jumlah;
        } else {
            daftarPembelian.push({
                id: ++counterId,
    ...
    function renderDaftar() {
        ...
        totalItemLabel.textContent = `${daftarPembelian.length} item`;
    ...
    function updatePembelian() {
        ...
        if (!supplierId) { alert('Silakan pilih supplier terlebih dahulu!'); return; }
        if (daftarPembelian.length === 0) { alert('Tambahkan minimal 1 barang!'); return; }
    ...
    function tambahSupplier() {
        ...
        if (!nama) { alert('Nama supplier tidak boleh kosong!'); return; }
